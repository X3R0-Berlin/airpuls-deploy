...
export function ProdukteContent({ products }: { products: Product[] }) {
  const { t } = useLanguage();
      ...
      <div className="max-w-[1100px] mx-auto px-[clamp(1.5rem,4vw,4rem)]">
        ...
        <BlurFade delay={0} inView>
          <nav className="flex items-center gap-1.5 text-[0.75rem] text-brand-text-muted mb-10">
            ...
            <span className="text-brand-text-dark">{t("page.products")}</span>
          </nav>
        </BlurFade>
                      ...
                      <div className="flex items-center justify-between">
                        {product.comingSoon ? (
                          ...
                          <span className="text-[0.95rem] font-semibold text-brand-text-dark">
                            {formatPrice(product.price, product.priceChf)}
                          </span>
                        )}
